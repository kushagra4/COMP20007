...
bool linear_hash_table_insert(LinearHashTable *table, int64 key) {
	...
	table->flag=5;
	printf("flag=%p", table->flag);
	table->temp_key=key;
	printf("t_key=%d table=%p", table->temp_key,table);
	printf(" key=%d h=%d temp_key=%d flag=%d table=%p\n", key,h,table->temp_key,table->flag,table);
...
void linear_hash_table_stats(LinearHashTable *table) {
	...
	printf("probes=%d\n",table->probes);
	//printf("average probe sequence=%d\n", table->probes/table->size);;
	printf("--- end stats ---\n");
}
